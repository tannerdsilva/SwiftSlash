...
writerchainpair_t wcp_init() {
	writerchainpair_t newpair = {
		.base = NULL,
		.tail = NULL
	};
	return newpair;
...
void wcp_close(const writerchainpair_ptr_t chain) {
	// load the base entry.
	writerchain_ptr_t getbase = atomic_load_explicit(chain->base, memory_order_acquire);
	if (getbase == NULL) {
...
bool wc_append_wc_ptr(const writerchainpair_ptr_t chain, writerchain_ptr_t newentry) {
	...
	writerchain_ptr_t currentTailEntry = atomic_load_explicit(chain->tail, memory_order_acquire);
	...
	if (storeResult == true) {
		...
		if (currentTailEntry == NULL) {
			atomic_store_explicit(chain->base, newentry, memory_order_release);
		} else {
			atomic_store_explicit(chain->tail, newentry, memory_order_release);
		}
	}
...
void wc_append(const writerchainpair_ptr_t chain, const data_ptr_t data, const size_t datalen) {
	...
	bool current = false;
	do {
		current = wc_append_wc_ptr(chain, newentry);
	} while (current == false);
}
...
bool wc_flush(const writerchainpair_ptr_t chain, const int fd, const err_ptr_t err) {
	const writerchain_aptr_ptr_t base = chain->base;
	const writerchain_aptr_ptr_t tail = chain->tail;
