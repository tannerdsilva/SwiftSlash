...
extension Collection {
	//explode a collection - no merging functionality
	public func explode(using thisFunction:@escaping (Int, Element) throws -> Void) {
		guard count > 0 else {
			return
		}

		let enumerateQueue = DispatchQueue(label:"com.tannersilva.function.explode.enumerate", target:process_master_queue)
