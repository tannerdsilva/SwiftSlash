...
extension NAsyncStream {
	...
	public final class Consumer {
		...
		private var fifoC:FIFO<Element, Failure>.Consumer
		private let il:IdentifiedList<FIFO<Element, Failure>>
...
extension NAsyncStream.Consumer where Failure == Never {
	...
	public func next(whenTaskCancelled cancelAction:consuming FIFO<Element, Failure>.Consumer.WhenConsumingTaskCancelled = .noAction) async -> Element? {
		return await fifoC.next(whenTaskCancelled:cancelAction)
	}
}

extension NAsyncStream.Consumer where Failure == Swift.Error {
	/// get the next element from the stream. this is a blocking call. if there is no data available, the call will block until data is available.
	/// - returns: the next element in the stream.
	public func next(whenTaskCancelled cancelAction:consuming FIFO<Element, Failure>.Consumer.WhenConsumingTaskCancelled = .noAction) async throws -> Element? {
